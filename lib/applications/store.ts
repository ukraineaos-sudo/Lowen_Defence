...
function dataToken(): string | undefined {
  return process.env.DATA_BLOB_READ_WRITE_TOKEN?.trim() || undefined;
}
...
export async function createApplication(
  ...
  if (token) {
    try {
      ...
      console.error("Application blob write failed:", err);
      return { success: false, error: "Не вдалося зберегти заявку" };
    }
  }
...
export async function listApplications(): Promise<CourseApplication[]> {
  ...
  if (token) {
    try {
      ...
      console.warn("Application blob list failed:", err);
    }
  }
...
): Promise<{ success: boolean; application?: CourseApplication; error?: string }> {
  const token = dataToken();
  if (token) {
    try {
      const found = await findBlobByAppId(id);
      if (!found) return { success: false, error: "Заявку не знайдено" };
      const application: CourseApplication = {
        ...found.app,
        status,
      ...
      console.error(err);
      return { success: false, error: "Не вдалося оновити заявку" };
    }
  }

  try {
    if (!fs.existsSync(LOCAL_APPS)) return { success: false, error: "Заявку не знайдено" };
    for (const file of fs.readdirSync(LOCAL_APPS).filter((f) => f.endsWith(".json"))) {
...
export async function deleteApplication(
  ...
  if (token) {
    try {
      ...
      return { success: false, error: "Не вдалося видалити заявку" };
    }
  }
